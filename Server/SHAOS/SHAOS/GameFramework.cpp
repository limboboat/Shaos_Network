...
void CGameFramework::MSG_Key(int playerID, UINT message, WPARAM wParam, LPARAM lParam)
{	
	if (pworld) pworld->MSG_Key(playerID, message, wParam, lParam);
}

void CGameFramework::MSG_Mouse(int playerID, UINT message, WPARAM wParam, LPARAM lParam, int iViewX)
{
	...
	if (PtInRect(&BUTTONPAUSE_RNG, mousepos)) {
		...
		if (pworld) pworld->MSG_Mouse(playerID, message, mousepos, iViewX);
	}
